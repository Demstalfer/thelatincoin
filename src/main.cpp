...
#if defined(NDEBUG)
# error "LatinCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa6ddffebc36b615a929bcd30ad9c5a5667f77edc6ad2b052bba9a913be6779dd");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // LatinCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "LatinCoin Signed Message:\n";
...
{
    // LatinCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // LatinCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // LatinCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1508563815, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "ElObservador102017GobiernoconfirmoquehaypetroleoenUruguay ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0464aa4c19b1ce01fa703efbedea88c18942c5dc94eba109a9ed6499833126fc2708c19d4ee8a4f31f9611f3428542c828e57e50f04f60eebdb12aa76e3179da10") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1508563815;
        ...
        assert(block.hashMerkleRoot == uint256("0x0d7bd32e9e022c1f8d059c5d21c6fa5752c7f139b93ea75f51cb7df5e5651cd8"));
...
unsigned char pchMessageStart[4] = { 0xb1, 0xe6, 0x85, 0xd0 }; // LatinCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LatinCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LatinCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LatinCoinMiner(CWallet *pwallet)
{
    printf("LatinCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("latincoin-miner");
    ...
    try { loop {
        ...
        printf("Running LatinCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LatinCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LatinCoinMiner, pwallet));
}
